...
export default class Render extends Component{
    ...
    _draw(exTime){
        ...
        let forwardList = {};
        let mats = {};
        this._m_Drawables.forEach(drawable=>{
            mats[drawable.getMaterial().getId()] = drawable.getMaterial();
            forwardList[drawable.getMaterial().getId()] = [drawable];
        });

        for(let matId in forwardList){
            forwardList[matId].forEach(geo=>{
                if(geo.isOpaque()){
                    // 获取当前选中的技术
                    let mat = mats[matId];
