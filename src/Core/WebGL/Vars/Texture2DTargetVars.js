 ...
 */
export default class Texture2DTargetVars extends Vars{
    // static _S_TEMP_COLOR = new UniformBufferI(4);
    static _S_TEMP_COLOR = new UniformBuffer(4);
    // 纹理滤波常量
    ...
    constructor(scene) {
        ...
        // 内部格式（默认为RGBA）
        this._m_Internalformat = gl.RGBA16F;
        // 外部格式（默认为RGBA）
        this._m_Format = gl.RGBA;
        // 数据类型
        this._m_Type = gl.FLOAT;
        // 数据更新标记
        ...
        this._m_WrapS = null;
        this._m_WrapT = null;
        this._m_MinFilter = Texture2DTargetVars.S_FILTERS.S_LINEAR_MIPMAP_NEAREST;
        this._m_MagFilter = Texture2DTargetVars.S_FILTERS.S_LINEAR;
    }
    ...
    setPreloadColor(scene, value){
        let color = Texture2DTargetVars._S_TEMP_COLOR.getArray();
        if (!value) {
            color[0] = 0;
            color[1] = 0;
            color[2] = 0;
    ...
    _upload(gl, loc, fun){
        ...
        if(this._m_Target){
            ...
            gl.bindFramebuffer(gl.READ_FRAMEBUFFER, this._m_Target.getFrameBuffer());
            // 某些图形驱动api规范仅支持2的
