...
export default class TilePassIBLLightingRenderProgram extends DefaultRenderProgram{
     ...
     * @private
     */
    _createTexture(scene, i, f, t){
        let tex = new Texture2DVars(scene);
        const gl = scene.getCanvas().getGLContext();
        tex.setTextureFormat(i, f, t);
        tex.setWrap(scene, Texture2DVars.S_WRAPS.S_CLAMP_TO_EDGE, Texture2DVars.S_WRAPS.S_CLAMP_TO_EDGE);
        tex.setFilter(scene, Texture2DVars.S_FILTERS.S_NEAREST, Texture2DVars.S_FILTERS.S_NEAREST);
        tex.updateTextureFilter();
        // 我们不需要读回主存,所以不需要进行对齐
        tex.setAlignment(1);
        return tex;
    }

    _reset(gl, scene, tileNum){
